...
test("should place letters correctly for a single platform", () => {
    const platforms = [
        Level.createLetteredWall(new AABB(new Vector2D(2, 1), 1, 3), ["A", "B", "C"]),
    ];
    const result = Level.createLetterMap(platforms);
...
test("should handle multiple platforms with letters", () => {
    const platforms = [
        Level.createLetteredWall(new AABB(new Vector2D(1, 1), 1, 2), ["X", "Y"]),
        Level.createLetteredWall(new AABB(new Vector2D(3, 2), 1, 3), ["A", "B", "C"]),
    ];
    const result = Level.createLetterMap(platforms);
...
test("should correctly handle platforms that exceed game boundaries", () => {
    const platforms = [
        Level.createLetteredWall(new AABB(new Vector2D(8, Window.GAME_HEIGHT - 1), 1, 4), ["W", "X", "Y", "Z"]),
    ];
    const result = Level.createLetterMap(platforms);
